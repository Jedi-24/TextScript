...
function outputText(sext) {
    ...
    const arr = parseLinks(sext.txt);
    if (arr.length !== 0) {
        arr.forEach(link => {
            sext.txt = (sext.txt).replace(link, `<a href=${link}  target ='_blank' >${link} </a>`);
        });
    }
...
function parseLinks(message) {
    const https = message.match(/\bhttps:\/\/\S+/gi);
    const http = message.match(/\bhttp:\/\/\S+/gi);
